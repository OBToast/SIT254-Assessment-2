...
@export var start_attack_range: float = 3.0
...
@export var parry_tutorial: bool = false
...
func _physics_process(delta: float) -> void:
	...
	if distance_to_player < start_attack_range:
...
func _on_attack_frame_changed() -> void:
	...
	# Parry window: open on the windup frame, close once the hit frame arrives
	if frame == 1:
		is_parryable = true
		if parry_tutorial and !has_parried:
	...
	if frame == 0 and swing_in_progress:
		...
		swing_in_progress = false
		attack_visual.visible = false
		is_parryable = false  # safety reset
...
func emit_shards(amount):
	for i in range(amount):
		...
		# Ignore collision with self instead of disabling the enemy's hitbox
		if shard.has_method("add_collision_exception_with"):
			shard.add_collision_exception_with(self)
...
func die():
	...
	tween.tween_method(_set_dissolve_amount, 0.0, 1.0, death_animation_length)
